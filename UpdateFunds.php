...
if($status == "paid") {
    ...
    if($payment) {
    	...
    	if( $payment_bonus ):
    		$insert= $insert->execute(array("c_id"=>$payment["client_id"],"action"=>$method_name." via API %".$payment_bonus["bonus_amount"]." bonus dahil ".$amount." balance loaded","ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
    	else:
    		$insert= $insert->execute(array("c_id"=>$payment["client_id"],"action"=>$method_name." via API ".$amount." balance loaded","ip"=>GetIP(),"date"=>date("Y-m-d H:i:s") ));
    	endif;
    	if( $update && $balance ):
    		$conn->commit();
    	else:
